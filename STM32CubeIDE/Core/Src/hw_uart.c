...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
    switch ((uint32_t)huart->Instance)
    {
        ...
        case (uint32_t)USART1:
        	...
        	// UART messaging for sending data
            if (uartRxBuffer[uartRxIndex] == '\n' || uartRxBuffer[uartRxIndex] == '\r')
            {
            	uartRxBuffer[uartRxIndex] = '\0';

            	if (strcmp((char*)uartRxBuffer, "data") == 0)
            	{
            		UARTSendData = 1;
            	}
            ...
            // Re-enable reception for next character
            HAL_UART_Receive_IT(&huart1, (uint8_t*)&uartRxBuffer[uartRxIndex], 1);
